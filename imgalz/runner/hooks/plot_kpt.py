...
class plot_kpt(plot):
    ...
    def after_iter(self, runner):
        ...
        kpts = result.get('kpt',[])
        for kpt in kpts:
            self.kpts(plot_img,kpt)
        runner.result['plot_img'] = plot_img


    def kpts(self,image,kpts, shape=(640, 640), radius=5, kpt_line=True):
        """
        ...
        if kpt_line:
            ...
            for i, sk in enumerate(self.skeleton):
                ...
                if pos2[0] % shape[1] == 0 or pos2[1] % shape[0] == 0 or pos2[0] < 0 or pos2[1] < 0:
                    continue
                cv2.line(image, pos1, pos2, [int(x) for x in self.limb_color[i]], thickness=2, lineType=cv2.LINE_AA)
